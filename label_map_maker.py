...
def item_maker(id: int, class_prefix: str) -> str:
    item = dict(id=id, class_prefix=class_prefix, class_name=str(id).zfill(ITEM_ZFILL))
...
@click.command()
@click.option("--label_map_output", type=click.Path(file_okay=True, dir_okay=False, writable=True), required=True, help="Path to label_map")
@click.option("--class_prefix", required=True, help="Prefix for the class_name i.g. 'HN'.")
